...
class ActivitySuggestionFlow(ChatFlow):
    ...
    def activity_memory_retriever(
        self, latest_exchanges_pretty, conversation_summary, user_id
    ):
        ...
        activity_memories = mem_manager.retrieve_activity_memories(
            user_situation, user_id
        )
        # mem_manager.close()
    ...
    def generate_response(self, conversation_state):
        ...
        try:
            ...
            user_situation_gauger_response, activity_memories = (
                self.activity_memory_retriever(
                    latest_exchanges_pretty, conversation_summary, user_id
                )
            )
            ...
            if activity_memories:
                chat_prompt_msgs.insert(
                    3, SystemMessage("Memories:\n " + activity_memories)
                )
            ...
            conversation_state.update(
                ...
                latest_as_flow_response=model_response,
                latest_user_situation_gauger_response=user_situation_gauger_response,
            ...
            conversation_state.update(
                latest_sakha_response=SakhaResponseForError(
                    ...
                    error=f"Error generating response in ASFlow {str(e)}",
                ),
                flow="activity_suggestion",
            )
            return conversation_state
