...
class FollowUpFlow(ChatFlow):
    ...
    def generate_response(self, conversation_state):
        ...
        try:
            ...
            if followup_start:
                chat_context = (
                    f"Conversation Summary:<summary>{conversation_summary}</summary>"
                )
            ...
            conversation_state.update(
                ...
                latest_fu_flow_response=model_response,
                flow="follow_up",
            )
            ...
            conversation_state.update(
                latest_sakha_response=SakhaResponseForError(
                    ...
                    error=f"Error generating response in FUFlow {str(e)}",
                ),
                flow="follow_up",
            )
            return conversation_state
