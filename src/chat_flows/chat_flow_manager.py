...
class ChatFlowManager:
    ...
    def get_chat_flow(self, conversation_state):
        ...
        if self.is_flow_change_valid(conversation_state):
            latest_supervisor_response = conversation_state.get(
                "latest_supervisor_response",
                SupervisorResponse(
    ...
    def is_flow_change_valid(self, conversation_state):
        ...
        if current_flow == "follow_up":
            latest_fu_flow_response = conversation_state.get(
                "latest_fu_flow_response",
                SakhaResponseForFUFlow(
                    replyToUser="Facing an error, please try again later",
                    isFeedbackCollectionComplete=False,
                    activityFeedback=None,
                ),
            )
            # print(latest_fu_flow_response)
            feedback_collection_complete = getattr(
                latest_fu_flow_response, "isFeedbackCollectionComplete", False
            )
