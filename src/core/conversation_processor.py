...
class ConversationProcessor:
    ...
    def process_input(
        self, user_input: str, thread_id="dummy_thread_id", user_id="dummy_user_id"
    ):
        ...
        try:
            ...
            to_user = cs.values.get(
                "to_user", "I'm sorry, I didn't quite understand that."
            )

            # Logging for debugging
            logger.info(
                f"[User: {user_id} | Thread: {thread_id}] Input: {user_input} | Response: {to_user}"
            )
            ...
            logger.error(
                f"Error processing input for user {user_id} in thread {thread_id}: {str(e)}",
                exc_info=True,
            )
